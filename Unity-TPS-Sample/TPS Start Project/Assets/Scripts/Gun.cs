...
public class Gun : MonoBehaviour
{
    ...
    public State state { get; private set; } // 총의 상태
    ...
    private LayerMask excludeTarget; // 총알을 쏴선 안되는 대상을 거르기 위한 마스크
    ...
    private void Awake()
    {
        ...
        // 총알이 나가는 지점과 탄착 지점 두개
        bulletLineRenderer.positionCount = 2;
        bulletLineRenderer.enabled = false; // bullerLineRenderer를 미처 비활성화하지 못한 경우 방지
    }
    ...
    {
        magAmmo = magCapacity; // 현재 탄창의 탄약을 가득 채워줌
        currentSpread = 0f; // 탄퍼짐의 정도 0에서 시작
        lastFireTime = 0f;
        state = State.Ready;
    }
    ...
    public bool Fire(Vector3 aimTarget)
    {
        if(state == State.Ready && Time.time >= lastFireTime + timeBetFire)
        {
            ...
            fireDirection = Quaternion.AngleAxis(yError, Vector3.up) * fireDirection; // Vector3.up방향으로 yError만큼 회전하는 Quaternion 생성
    ...
    private void Shot(Vector3 startPoint, Vector3 direction)
    {
        ...
        {
            var target = hit.collider.GetComponent<IDamageable>(); // 충돌한 collider가 IDamagable로써 가져올 수 있는가
    ...
    private IEnumerator ShotEffect(Vector3 hitPosition)
    {
        ...
        gunAudioPlayer.PlayOneShot(shotClip); // 소리 중첩을 위해 Play메소드 대신 PlayOneShot 메소드 사용

        bulletLineRenderer.enabled = true;
        bulletLineRenderer.SetPosition(0, fireTransform.position);
        bulletLineRenderer.SetPosition(1, hitPosition);

        yield return new WaitForSeconds(0.03f); // 0.03초동안만 bulletLineRenderer활성화
    ...
    private IEnumerator ReloadRoutine()
    {
        ...
        var ammoToFill = Mathf.Clamp(magCapacity - magAmmo, 0, ammoRemain); // Math.Clamp 값을 ammoRemain보다 작은 값으로 잘라줌
